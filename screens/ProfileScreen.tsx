      ...
      <main className="flex-1 overflow-y-auto pb-24 no-scrollbar">
        <div className="flex flex-col items-center py-8 px-4 bg-surface-light dark:bg-surface-dark">
          <div className="relative">
            <div 
              className="size-28 rounded-full border-4 border-primary/20 bg-cover bg-center shadow-lg"
              style={{ backgroundImage: `url("https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=200")` }}
            ></div>
          ...
          <h3 className="mt-4 text-2xl font-bold text-[#111418] dark:text-white">Alex Silva</h3>
          <p className="text-gray-500 font-medium text-sm">Viajante Enthusiasta • @alex_travels</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 p-4">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-white dark:bg-[#1e2a36] p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col items-center gap-1">
              <span className="material-symbols-outlined text-primary text-[24px] mb-1">{stat.icon}</span>
        ...
        <div className="px-4 py-2 space-y-3">
          <h4 className="text-sm font-bold text-gray-400 uppercase tracking-widest px-2">Preferências</h4>
          <div className="bg-white dark:bg-[#1e2a36] rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800">
            {[
              ...
              <div key={item.label} className={`flex items-center justify-between p-4 ${idx !== 0 ? 'border-t border-gray-50 dark:border-gray-800' : ''}`}>
                ...
                {item.toggle ? (
                  <div className="w-10 h-5 bg-primary/20 rounded-full relative cursor-pointer">
                    <div className="absolute top-0.5 left-5 w-4 h-4 bg-primary rounded-full shadow-sm"></div>
                  </div>
                ) : item.value ? (
        ...
        <div className="px-4 py-4">
           <button className="w-full bg-red-50 text-red-500 dark:bg-red-500/10 dark:text-red-400 font-bold py-4 rounded-2xl flex items-center justify-center gap-2 active:bg-red-100 transition-colors">
