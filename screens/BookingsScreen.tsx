...
// Definição dos tipos locais para este componente
interface BookingItem {
  ...
  tripName: string;
  tripDate: string;
  // Propriedades específicas para renderização
  data: any; 
}
...
const BookingsScreen: React.FC = () => {
  ...
  const openBoardingPass = (item: any) => {
    setSelectedItem(item);
    setModalType('boardingPass');
  };
        ...
        <div className="bg-gray-100 dark:bg-gray-800 p-1 rounded-xl flex mb-3">
          {[
            ...
            <button 
              ...
              className={`flex-1 h-8 rounded-lg text-[12px] font-bold transition-all ${activeTab === tab.id ? 'bg-white dark:bg-surface-dark shadow-sm text-black dark:text-white' : 'text-gray-400'}`}
            >
              {tab.label}
            </button>
          ))}
      ...
      <main className="flex-1 overflow-y-auto px-4 pb-24 no-scrollbar pt-4 space-y-6">
        {Object.keys(groupedBookings).length > 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {groupedBookings[tripName].map(booking => {
                  ...
                  if (booking.type === 'flights') {
                    return (
                      <div key={booking.id} className="bg-white dark:bg-surface-dark rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800">
                          ...
                          <div className="flex items-center gap-3">
                             ...
                             <div>
                               <div className="flex items-center gap-1.5">
                                 <span className="text-[13px] font-bold text-[#111418] dark:text-white">{d.origin}</span>
                                 <span className="material-symbols-outlined text-[14px] text-gray-400">arrow_forward</span>
                                 <span className="text-[13px] font-bold text-[#111418] dark:text-white">{d.dest}</span>
                               </div>
                               <p className="text-[10px] text-gray-400 font-medium">{d.airline} • {d.flightNum}</p>
                             </div>
                          </div>
                          <span className={`px-2.5 py-1 rounded-full border text-[10px] font-bold ${booking.status === 'upcoming' ? 'border-green-200 bg-green-50 text-green-600' : 'border-gray-200 bg-gray-50 text-gray-500'}`}>
                            {d.statusText}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between items-center relative mb-6">
                           ...
                           <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-2">
                              <span className="material-symbols-outlined text-gray-400 text-[16px] rotate-90">flight</span>
                              <p className="text-[9px] text-center text-gray-400 font-bold mt-1">{d.isDirect ? 'Direto' : 'Escala'}</p>
                           </div>
                           ...
                           <div className="text-center relative z-10 bg-white dark:bg-surface-dark pl-2">
                             <p className="text-[20px] font-black text-[#111418] dark:text-white leading-none">{d.departureTime}</p>
                             <p className="text-[11px] font-bold text-gray-400 uppercase">{d.originCode}</p>
                             <p className="text-[10px] text-gray-400 mt-1">{d.departureDate}</p>
                           </div>
                           <div className="text-center relative z-10 bg-white dark:bg-surface-dark pr-2">
                             <p className="text-[20px] font-black text-[#111418] dark:text-white leading-none">{d.arrivalTime}</p>
                             <p className="text-[11px] font-bold text-gray-400 uppercase">{d.destCode}</p>
                             <p className="text-[10px] text-gray-400 mt-1">{d.arrivalDate}</p>
                           </div>
                        </div>

                        <div className="pt-4 border-t border-gray-100 dark:border-gray-800 text-center">
                           <button onClick={() => openBoardingPass(d)} className="text-[13px] font-bold text-blue-500 hover:text-blue-600 transition-colors">Ver Cartão de Embarque</button>
                        </div>
                      </div>
                    );
                  }
                  ...
                  if (booking.type === 'hotels') {
                    return (
                      <div key={booking.id} className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800">
                         ...
                         <div className="flex gap-3 mb-4">
                            <div className="flex-1 bg-gray-50 dark:bg-gray-800 rounded-lg p-2.5">
                               <p className="text-[8px] font-bold text-gray-400 uppercase">CHECK-IN</p>
                               <p className="text-[11px] font-bold text-[#111418] dark:text-white">{d.checkIn}</p>
                            </div>
                            <div className="flex-1 bg-gray-50 dark:bg-gray-800 rounded-lg p-2.5">
                               <p className="text-[8px] font-bold text-gray-400 uppercase">CHECK-OUT</p>
                               <p className="text-[11px] font-bold text-[#111418] dark:text-white">{d.checkOut}</p>
                            </div>
                         </div>

                         <div className="flex items-center gap-1 text-blue-500">
                            <span className="material-symbols-outlined text-[16px] material-symbols-filled">map</span>
                            <button onClick={() => openMap(d)} className="text-[11px] font-bold hover:underline">Ver no mapa</button>
                         </div>
                      </div>
                    );
                  }
                  ...
                  // RENDER ACTIVITY
                  if (booking.type === 'activities') {
                    return (
                      <div key={booking.id} onClick={() => openDetails(d)} className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all">
                         <div className="flex gap-4">
                            <div className="size-12 rounded-xl bg-red-50 flex items-center justify-center shrink-0">
                               <span className="material-symbols-outlined text-red-500">local_activity</span>
                            </div>
                            <div className="flex flex-col justify-center">
                               <h4 className="text-[13px] font-bold text-[#111418] dark:text-white">{d.name}</h4>
                               <p className="text-[10px] text-gray-500 mt-0.5">{d.date}</p>
                               <p className="text-[10px] text-gray-400 mt-0.5">{d.details}</p>
                            </div>
                         </div>
                      ...
                      <div key={booking.id} className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800">
                         <div className="flex justify-between items-start mb-4">
                           <div className="flex gap-4">
                              ...
                              <div>
                                 <h4 className="text-[13px] font-bold text-[#111418] dark:text-white">{d.name}</h4>
                                 <p className="text-[11px] text-gray-500">{d.model}</p>
                              </div>
                           </div>
                           <span className="px-2.5 py-1 rounded-full border border-yellow-200 bg-yellow-50 text-yellow-700 text-[10px] font-bold">{d.statusText}</span>
                         </div>

                         <div className="flex items-center justify-between">
                            <div>
                               <p className="text-[11px] font-bold text-[#111418] dark:text-white">Retirada: {d.pickupDate}</p>
                               <p className="text-[10px] text-gray-400 leading-tight max-w-[80px] truncate">{d.pickupLoc}</p>
                            </div>
                            <span className="material-symbols-outlined text-gray-300">arrow_forward</span>
                            <div className="text-right">
                               <p className="text-[11px] font-bold text-[#111418] dark:text-white">Devolução: {d.dropoffDate}</p>
                               <p className="text-[10px] text-gray-400 leading-tight max-w-[80px] truncate ml-auto">{d.dropoffLoc}</p>
                            </div>
                         </div>
                      </div>
                    );
          ...
          <div className="flex flex-col items-center justify-center pt-20">
            <span className="material-symbols-outlined text-gray-300 text-[48px] mb-2">event_busy</span>
            <p className="text-gray-400 font-bold text-sm">Nenhuma reserva encontrada</p>
          </div>
        )}
      </main>

      {/* FAB - Adicionar Reserva */}
      ...
      {modalType !== 'none' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-sm bg-white dark:bg-surface-dark rounded-3xl overflow-hidden shadow-2xl relative animate-slide-up">
            <button onClick={() => setModalType('none')} className="absolute top-4 right-4 z-10 size-8 bg-black/10 dark:bg-white/10 rounded-full flex items-center justify-center hover:bg-black/20 transition-colors">
              <span className="material-symbols-outlined text-[18px]">close</span>
            </button>

            {modalType === 'boardingPass' && selectedItem && (
              <div className="flex flex-col">
                <div className="bg-blue-600 p-6 text-white text-center pt-10 pb-8">
                  <h3 className="text-[18px] font-bold mb-1">Cartão de Embarque</h3>
                  <p className="opacity-80 text-[12px]">{selectedItem.airline}</p>
                </div>
                <div className="p-6 bg-white dark:bg-surface-dark relative">
                   ...
                   <div className="flex justify-between items-center mb-6">
                      <div className="text-center">
                        <h1 className="text-3xl font-black text-primary">{selectedItem.originCode}</h1>
                        <p className="text-[10px] text-gray-500">{selectedItem.origin}</p>
                      </div>
                      <span className="material-symbols-outlined text-gray-300">flight_takeoff</span>
                      <div className="text-center">
                        <h1 className="text-3xl font-black text-primary">{selectedItem.destCode}</h1>
                        <p className="text-[10px] text-gray-500">{selectedItem.dest}</p>
                      </div>
                   </div>
                   <div className="space-y-4">
                     <div className="flex justify-between border-b border-gray-100 pb-2">
                       <span className="text-[11px] text-gray-400 font-bold">Voo</span>
                       <span className="text-[13px] font-bold">{selectedItem.flightNum}</span>
                     </div>
                     <div className="flex justify-between border-b border-gray-100 pb-2">
                       <span className="text-[11px] text-gray-400 font-bold">Embarque</span>
                       <span className="text-[13px] font-bold">{selectedItem.departureTime}</span>
                     </div>
                     <div className="flex justify-between border-b border-gray-100 pb-2">
                       <span className="text-[11px] text-gray-400 font-bold">Portão</span>
                       <span className="text-[13px] font-bold">B12</span>
                     </div>
                   </div>
                   <div className="mt-8 flex justify-center">
                      <div className="h-12 w-full bg-gray-200 flex items-center justify-center text-[10px] font-mono tracking-[4px]">||| || ||| || |||| ||</div>
                   </div>
                </div>
              </div>
            )}

            {modalType === 'map' && selectedItem && (
              <div className="flex flex-col h-[400px]">
                <div className="flex-1 bg-gray-200 relative">
                  <img src="https://images.unsplash.com/photo-1524661135-423995f22d0b?auto=format&fit=crop&q=80&w=600" className="w-full h-full object-cover" alt="Map" />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="material-symbols-outlined text-red-500 text-[48px] drop-shadow-md">location_on</span>
                  </div>
                </div>
                <div className="p-4 bg-white dark:bg-surface-dark">
                  <h3 className="font-bold text-[14px]">{selectedItem.name}</h3>
                  <p className="text-[11px] text-gray-500 mt-1">Clique para abrir no Google Maps</p>
                  <button className="mt-3 w-full bg-primary text-white font-bold py-3 rounded-xl text-[12px]">Navegar</button>
                </div>
              </div>
            )}
            ...
            {modalType === 'details' && selectedItem && (
              <div className="p-6 pt-10">
                <div className="size-16 rounded-2xl bg-red-50 flex items-center justify-center mb-4 mx-auto">
                   <span className="material-symbols-outlined text-red-500 text-[32px]">local_activity</span>
                </div>
                <h3 className="text-[18px] font-bold text-center mb-2">{selectedItem.name}</h3>
                <p className="text-[12px] text-gray-500 text-center mb-6">{selectedItem.details}</p>
                ...
                <button className="w-full h-12 bg-primary text-white font-bold rounded-xl text-[13px]">
                  Ver Voucher
                </button>
              </div>
            )}
